...
def getSongs(playerURL, songNum, pageNum):
    global activeThreads, distribute_count
    while (activeThreads != 0) or (distribute_count < int(songs)):
        i = 0
        for x in range(1,pageNum+1):
            url = "https://scoresaber.com/u/{}".format(playerURL).split("&", 1)[0]+"&page={}&sort=2".format(x)
            page = requests.get(url)
            tree = html.fromstring(page.content)
            rows = tree.xpath('//div/table/tbody/tr/th/div/div/img')
            for row in rows:
                if distribute_count < int(songNum):
                    print('Starting download '+str(distribute_count+1)+'/'+songNum)
